...
function Chat({ messages: chatMessages, onSendMessage }) {
  ...
  return (
    <div className="flex min-h-full flex-col gap-4">
      ...
      <div className="flex items-center gap-2 rounded-3xl bg-white p-2 shadow-sm focus-within:ring-2 focus-within:ring-inset focus-within:ring-amber-500">
        <input aria-label="Сообщение кухне" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => event.key === "Enter" && sendMessage()} className="h-12 min-w-0 flex-1 bg-transparent px-3 font-semibold outline-none placeholder:text-slate-400" placeholder="Сообщение кухне..." />
...
function SearchBox({ value, onChange }) {
  return (
    <label className="flex h-14 items-center gap-3 rounded-3xl bg-white px-4 shadow-sm focus-within:ring-2 focus-within:ring-inset focus-within:ring-amber-500">
      <Search className="text-slate-400" size={22} />
      <input aria-label="Поиск" value={value} onChange={(event) => onChange(event.target.value)} className="h-full min-w-0 flex-1 bg-transparent text-base font-semibold outline-none placeholder:text-slate-400" placeholder="Блюдо, аллерген, цех..." />
    </label>
  );
}
