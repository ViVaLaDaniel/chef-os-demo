  ...
  return (
    <div className="flex min-h-full flex-col gap-4">
      ...
      <div className="flex items-center gap-2 rounded-3xl bg-white p-2 shadow-sm focus-within:ring-2 focus-within:ring-amber-500 transition-shadow">
        <input aria-label="Текст сообщения" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => event.key === "Enter" && sendMessage()} className="h-12 min-w-0 flex-1 bg-transparent px-3 font-semibold outline-none placeholder:text-slate-400" placeholder="Сообщение кухне..." />
        <button onClick={sendMessage} className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-amber-500 text-white focus-visible:ring-2 focus-visible:ring-amber-600 focus-visible:ring-offset-2 outline-none transition-shadow disabled:bg-slate-200" aria-label="Отправить" disabled={!draft.trim()}>
...
function SearchBox({ value, onChange }) {
  return (
    <label className="flex h-14 items-center gap-3 rounded-3xl bg-white px-4 shadow-sm focus-within:ring-2 focus-within:ring-amber-500 transition-shadow">
      <Search className="text-slate-400" size={22} aria-hidden="true" />
      <input aria-label="Поиск по ТТК" value={value} onChange={(event) => onChange(event.target.value)} className="h-full min-w-0 flex-1 bg-transparent text-base font-semibold outline-none placeholder:text-slate-400" placeholder="Блюдо, аллерген, цех..." />
    </label>
  );
}
